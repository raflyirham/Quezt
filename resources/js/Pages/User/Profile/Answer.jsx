        ...
        {answers.data.length > 0 ? (
          <>
            {answers.data.map((answer, index) => (
              <ProfileContentCard
                key={index}
                link={`/pertanyaan/${answer.questionanswers[0].questionheader.QuestionID}`}
                title={""}
                detail={answerSlice(answer.answerdetail.AnswerDetail)}
                courseName={
                  answer.questionanswers[0].questionheader.course.CourseName
                }
                majorName={
                  answer.questionanswers[0].questionheader.course.major
                    .MajorName
                }
                date={convertDate(answer.answerdetail.AnswerDate)}
