        ...
        {questions.data.length > 0 ? (
          <>
            {questions.data.map((question, index) => (
              <ProfileContentCard
                key={index}
                link={`/pertanyaan/${question.QuestionID}`}
                title={question.questiondetail.QuestionTitle}
                detail={questionSlice(
                  deleteHTMLTag(question.questiondetail.QuestionDetail),
                )}
                courseName={question.course.CourseName}
                majorName={question.course.major.MajorName}
                date={convertDate(question.questiondetail.QuestionDate)}
